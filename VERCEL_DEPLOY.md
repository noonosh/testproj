...
1. The frontend (React + Vite) is built and served as static files
2. API routes (`/trpc/*`) are handled by serverless functions in `api/index.ts`
3. The `vercel.json` rewrites `/trpc/*` requests to `/api/trpc/*`
4. The Hono server handles tRPC requests
