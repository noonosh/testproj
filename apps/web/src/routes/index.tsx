...
function HomeComponent() {
	...
	const isValidExpression = (expr: string): boolean => {
		// Must contain at least one number or valid constant
		...
		// Remove all valid functions and constants
		...
		// Remove valid function names
		for (const fn of validFunctions) {
			cleaned = cleaned.replace(new RegExp(fn, "g"), "")
		}

		// Remove numbers, operators, spaces, parentheses, commas, dots
		cleaned = cleaned.replace(/[0-9+\-*/().\s^,]/g, "")

		// If there are any remaining characters, they're invalid
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			// Don't show error UI for rate limit errors (toast handles it)
			const errorMessage =
		...
		<div className="min-h-screen bg-gray-50 dark:bg-gray-950">
			<div className="container mx-auto max-w-4xl px-4 py-12">
				...
				<div className="text-center mt-16 text-xs text-gray-400 dark:text-gray-600">
					<p className="font-mono">Better-T-Stack × Califi</p>
				</div>
			</div>
		</div>
	)
